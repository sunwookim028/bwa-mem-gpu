...
/* second set of reads pointer to process
	when processing, gpu_ptrs_t gpu_data should have this set
 */
extern bseq1_t *preallocated_seqs2;		// a chunk of SEQ_MAX_COUNT seqs on host's pinned memory
extern bseq1_t *d_preallocated_seqs2;	// same chunk on device
extern char *seq_name_ptr2, *seq_comment_ptr2, *seq_ptr2, *seq_qual_ptr2, *seq_sam_ptr2;		// pointers to chunks on host
extern char *d_seq_name_ptr2, *d_seq_comment_ptr2, *d_seq_ptr2, *d_seq_qual_ptr2;			// pointers to chunks on device